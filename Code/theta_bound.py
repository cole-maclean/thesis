...
if __name__ == '__main__':
	...
	for N in N_list:
		if N in limits_dict.keys():
			for R in range(R_range[0],R_range[1],R_step):
				if str(R) in limits_dict[N].keys():
					sim_params.append([N,R,limits_dict[N][str(R)],iterations,save_file])
				else:
					sim_params.append([N,R,0.00001,iterations,save_file])
		else:
			for R in range(R_range[0],R_range[1],R_step):
				sim_params.append([N,R,0.00001,iterations,save_file])
	p = mp.Pool(n_jobs)
	for rslt in tqdm.tqdm(p.imap_unordered(simulator.upper_bound_theta, sim_params,chunksize=1), total=len(sim_params),smoothing=0):
		if rslt[0] in limits_dict.keys():
			limits_dict[rslt[0]][str(rslt[1])] = rslt[2]
		else:
			limits_dict[rslt[0]] = {str(rslt[1]):rslt[2]}
